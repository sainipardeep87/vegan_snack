...
=end
    ...
    def get_related_product
      product_id = Spree::Variant.select('product_id').where(id: self.variant_id).first.product_id
      Spree::Product.where(id: product_id).first
    end
    ...
    def self.no_associated_orders?(order_id)
      LineItem.where(order_id: order_id).blank?
    end
