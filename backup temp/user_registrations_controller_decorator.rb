...
session[:registration_params].deep_merge!(params[:spree_user]) if params[:spree_user]
@user = Spree::User.new(session[:registration_params]) 

@user.current_step = session[:registration_step]
puts "the currenty#{@user.current_step}"
# if @user.current_step=="billing"
  
  # end
 if @user.valid?
  if params[:back_button]
    puts "i am in backbutton"
        @user.previous_step
      elsif @user.current_step=="billing"
         # @user.save if @user.all_valid?
         
          sub_id =session[:registration_params][:sub_type].blank? ? "1" : session[:registration_params][:sub_type]
    coupon_code = session[:registration_params][:coupon_code]
    @cart = Cart.new
    ...
    if @user.valid?
      ...
      if @success && @user.save
       ...
       @user.remove_errormessages_added_by_spree
         ...
         puts "i am in last step"
      else
        puts "i am in next step"
        @user.next_step
 ...
 if @user.new_record?
  ...
  def new
    puts "i am in new with params#{request.env['omniauth.auth'].present?}"
